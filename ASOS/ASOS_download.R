...
# This code borrowed from https://github.com/realmiketalbot/Public/blob/master/iem_scraper_example.r
...
sites <- readRDS(file.path(datadir, AK_ASOS_sites.rds))
iem.wd <- datadir #download location
date1 <- ISOdate(1970,1,1) #start date in year, month, day format
date2 <- ISOdate(2019,5,28) #end date in year, month, day format
user.network <- c("ASOS")
user.state <- c("AK") #state
user.faaid <- c("PATO") #site FAA identifier - leave empty and a list will print for your reference
#################
...
# select wind direction, speed, and for peak gusts
service <- str_c(service, "data=drct&data=sped&data=gust&data=peak_wind_gust&data=peak_wind_drct&data=peak_wind_time&tz=Etc/UTC&format=comma&latlon=yes&", sep="")
